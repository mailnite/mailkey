...
```text
GET https://mail.<d>/.well-known/mail-key-identity
Content-Type: application/msgpack
```

The body is a canonical object carrying the active identity key, its `fp`,
status, and the ordered chain of rotation statements (§5). A client fetches it
only to establish a pin, to follow a rotation, or to check revocation — never
