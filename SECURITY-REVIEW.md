...
| F-4 | Medium | Downgrade protection vs. observation-driven self-DoS | RESOLVED both ways. `peer` never lets a failed refresh disturb a valid manifest, and `ResolveForEncryption` falls back to cache. Under `PolicyRequire` a failure now reports `ErrEncryptionRequired` (wrapping the cause), the resolver adapter passes it through, and the queue refuses the whole submission BEFORE storing anything — `api.ErrEncryptionRequired` → SMTP **451**, so the mail waits rather than leaking in cleartext or bouncing. A permanent rejection here would have let anyone able to make an authority unreachable destroy mail instead of delaying it |
